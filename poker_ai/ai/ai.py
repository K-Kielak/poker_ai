...
def update_strategy(
    ...
    """
    ph = state.player_i  # this is always the case no matter what player_i is

    player_not_in_hand = not state.players[player_i].is_active
    ...
    #   update_strategy(rs, h + a, player_i, iteration)
        ...
        update_strategy(agent, new_state, player_i, iteration, locks)
    else:
...
def cfr(
    ...
    log.debug(f"Player Set to Update Regret: {player_i}")
    log.debug(f"P(h): {state.player_i}")
    log.debug(f"P(h) Updating Regret? {state.player_i == player_i}")
    ...
    if ph == player_i:
        ...
        for action in state.legal_actions:
            ...
            new_state: ShortDeckPokerState = state.apply_action(action)
            voa[action] = cfr(agent, new_state, player_i, iteration, locks)
    ...
    log.debug(f"ACTION SAMPLED: ph {state.player_i} ACTION: {action}")
    new_state: ShortDeckPokerState = state.apply_action(action)
    return cfr(agent, new_state, player_i, iteration, locks)
...
def cfrp(
    ...
    if ph == player_i:
        ...
        for action in state.legal_actions:
            if this_info_sets_regret[action] > c:
                new_state: ShortDeckPokerState = state.apply_action(action)
                voa[action] = cfrp(agent, new_state, player_i, iteration, c, locks)
    ...
    action: str = np.random.choice(available_actions, p=action_probabilities)
    new_state: ShortDeckPokerState = state.apply_action(action)
    return cfrp(agent, new_state, player_i, iteration, c, locks)
